...
class SingleModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.input_C = self.Tensor(nb, opt.input_nc, size, size)
        self.input_D = self.Tensor(nb, opt.output_nc, size, size)
        self.input_img = self.Tensor(nb, opt.input_nc, size, size)
        self.input_img1 = self.Tensor(nb, opt.input_nc, size, size)
        ...
        if opt.vgg > 0:
            ...
            self.contrastloss=networks.ContrastLoss(opt)
            if self.opt.IN_vgg:
    ...
    def set_input(self, input):
        ...
        input_C = input['C' if AtoB else 'D']
        input_D = input['D' if AtoB else 'C']

        input_img = input['input_img']
        input_img1 = input['input_img1']
        ...
        self.input_D.resize_(input_D.size()).copy_(input_D)
        self.input_img1.resize_(input_img1.size()).copy_(input_img1)
        self.image_paths = input['A_paths' if AtoB else 'B_paths']
        self.image_paths1 = input['C_paths']
    ...
    def get_image_paths(self):
        return self.image_paths, self.image_paths1
    ...
    def backward_G(self, epoch):
        ...
        # self.real_C, self.latent_real_A = self.netG_A.forward(self.real_A, self.real_A_gray)
        #
        # self.loss_G_A1 = self.criterionIdt(self.real_B, self.real_C) * 5
        if self.opt.use_wgan:
        ...
        self.real_B = Variable(self.input_B)
        self.fake_D, self.c = self.netG_A.forward(self.real_C, self.real_C_gray)
        self.l1=nn.L1Loss()
        self.fake_D.detach()
        self.loss_G_A1 = 1 - self.ssim(self.real_B, self.fake_D)
        # self.loss_G_A1=self.l1(self.fake_D,self.real_B)
        self.loss_G = self.loss_G + self.loss_G_A1
    ...
    def optimize_parameters(self, epoch):
        ...
        self.optimizer_G.zero_grad()
        self.backward_G(epoch)
        nn.utils.clip_grad_norm(self.loss_G_A, max_norm=1)
        self.optimizer_G.step()
        # D_A
        self.optimizer_D_A.zero_grad()
        self.backward_D_A()
        nn.utils.clip_grad_norm(self.loss_D_A, max_norm=1)
    ...
    def get_current_errors(self, epoch):
        ...
        # G_A1 = self.loss_G_A1.data[0]
        if self.opt.vgg > 0:
            vgg = self.loss_vgg_b.data[0] / self.opt.vgg if self.opt.vgg > 0 else 0
            return OrderedDict([('D_A', D_A), ('G_A', G_A),  ("D_P", D_P)])
        elif self.opt.fcn > 0:
    ...
    def get_current_visuals(self):
        ...
        if self.opt.skip > 0:
            ...
            if self.opt.patchD:
                ...
                if self.opt.patch_vgg:
                    ...
                    if not self.opt.self_attention:
                        return OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('fake_D', fake_D),
                                            ('latent_real_A', latent_real_A),
                                            ('latent_show', latent_show), ('real_B', real_B), ('real_C', real_C),
                    ...
                    if not self.opt.self_attention:
                        ...
                        return OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('fake_D', fake_D),
                                            ...
                                            ('real_patch', real_patch),
                                            ('fake_patch', fake_patch),
                                            ('real_C', real_C)('self_attention', self_attention)])
            else:
                if not self.opt.self_attention:
                    return OrderedDict(
                        [('real_A', real_A), ('fake_B', fake_B), ('fake_D', fake_D), ('latent_real_A', latent_real_A),
                         ('latent_show', latent_show), ('real_B', real_B)])
                else:
                    self_attention = util.atten2im(self.real_A_gray.data)
                    return OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('fake_D', fake_D), ('real_B', real_B),
            ...
            if not self.opt.self_attention:
                ...
                self_attention = util.atten2im(self.real_A_gray.data)
                return OrderedDict(
                    [('real_A', real_A), ('real_C', real_C), ('fake_D', fake_D), ('fake_B', fake_B), ('real_B', real_B),
                     ('self_attention', self_attention)])
