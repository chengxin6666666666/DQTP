...
import sys
from hyperIQA import hyperIQA_models
...
class SingleModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG_B = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.which_model_netG, opt.norm,
                                        not opt.no_dropout, self.gpu_ids, skip=skip, opt=opt)
        ...
        if self.isTrain:
            ...
            if self.opt.patchD:
                ...
                self.netD_P1 = networks.define_D(opt.input_nc, opt.ndf,
                                                 opt.which_model_netD,
                                                 opt.n_layers_patchD, opt.norm, use_sigmoid, self.gpu_ids, True)
        if not self.isTrain or opt.continue_train:
            which_epoch = opt.which_epoch
            self.load_network(self.netG_B, 'G_A', '380')
            self.load_network(self.netG_A, 'G_A', '200')


            if self.isTrain:
                self.load_network(self.netD_B, 'D_A', '380')
                self.load_network(self.netD_A, 'D_A', '200')
                if self.opt.patchD:
                    self.load_network(self.netD_P1, 'D_P', '380')
                    self.load_network(self.netD_P, 'D_P', '200')
        if self.isTrain:
            self.old_lr = opt.lr
    ...
    def get_phase_loss(self, img1, img2):
        ...
        fft1 = torch.rfft(img1, signal_ndim=2, onesided=False, normalized=True)  # fft tranform
        fft2 = torch.rfft(img2, signal_ndim=2, onesided=False, normalized=True)  # fft tranform
    ...
    def transforms_crop(self, real_A_tensor, fake_B_tensor):
        position_H = random.randint(0, 192)
        position_W = random.randint(0, 192)
        real_A_patch = real_A_tensor[:, :, position_H:position_H + 64, position_W:position_W + 64]
        fake_B_patch = fake_B_tensor[:, :, position_H:position_H + 64, position_W:position_W + 64]
        return real_A_patch, fake_B_patch

    def get_quality_hyperIQA_loss(self, img1, img2, epoch):
        if epoch < 2:
            self.model_hyper = hyperIQA_models.HyperNet(16, 112, 224, 112, 56, 28, 14, 7).cuda()
            self.model_hyper.train(False)
            self.model_hyper.load_state_dict((torch.load('./hyperIQA/hyperIQA_pretrained/koniq_pretrained.pkl')))
        # import pdb; pdb.set_trace()

        img1 = F.interpolate(img1, size=(224, 224), mode='bilinear')
        img2 = F.interpolate(img2, size=(224, 224), mode='bilinear')
        # for i in range(1):
        paras1 = self.model_hyper(img1)
        # Building target network
        model_target1 = hyperIQA_models.TargetNet(paras1).cuda()
        for param in model_target1.parameters():
            param.requires_grad = False
        pred1 = model_target1(paras1['target_in_vec'])
        # pred1 = torch.mean(pred1, dim=0)

        paras2 = self.model_hyper(img2)
        model_target2 = hyperIQA_models.TargetNet(paras2).cuda()

        for param in model_target2.parameters():
            param.requires_grad = False
        pred2 = model_target2(paras2['target_in_vec'])
        hyperIQA_score = abs(pred1 - pred2) /( pred2)

        return torch.mean(hyperIQA_score, dim=0)

    def get_task_loss(self, fake_img, epoch):
        if epoch < 2:
            task_model = Pix2PixModel()
            load_filename = 'latest_net_G.pth'
            load_path = './daytime_inference/checkpoints/daytimepredict_LOL/'
            ...
            self.task_net.load_state_dict(_state_dict)

            # print('\nloading the model paras from {} on device--{}\n'.format(load_file, _device))
        daytime_tensor_fake_B = torch.mean(abs(self.task_net(fake_img)), dim=0)

        return ((daytime_tensor_fake_B ) ).mean() / 2.
    def get_task_loss1(self, fake_img, epoch):
        if epoch < 2:
            task_model = Pix2PixModel()
            load_filename = 'latest_net_G.pth'
            load_path = './daytime_inference/checkpoints/daytimepredict_LOL/'
        ...
        return (-1*abs(daytime_tensor_fake_B )+ 1).mean() / 2.
    ...
    def forward(self):
        ...
        self.fake_C = self.netG_A.forward(self.real_B, self.real_B_gray)
        self.fake_A = self.netG_B.forward(self.real_B, self.real_B_gray)
        self.fake_B = self.netG_A.forward(self.real_img, self.real_A_gray)
        # self.fake_B = torch.squeeze(self.fake_B)
        # r, g, b = self.fake_B[0] + 1, self.fake_B[1] + 1, self.fake_B[2] + 1
        # self.fake_B_gray = 1. - (0.299 * r + 0.587 * g + 0.114 * b) / 2.
        # self.fake_B_gray = torch.unsqueeze(self.fake_B_gray, 0)
        # self.fake_B_gray = torch.unsqueeze(self.fake_B_gray, 0)
        # self.fake_B = torch.unsqueeze(self.fake_B, 0)
        #
        # self.fake_B2 = torch.squeeze(self.fake_B2)

        self.fake_B3, self.fake_B2 = torch.split(self.fake_B, [1, 1], dim=0)

        self.fake_B3 = torch.squeeze(self.fake_B3)
        r, g, b = self.fake_B3[0] + 1, self.fake_B3[1] + 1, self.fake_B3[2] + 1
        self.fake_B3_gray = 1. - (0.299 * r + 0.587 * g + 0.114 * b) / 2.
        self.fake_B3_gray = torch.unsqueeze(self.fake_B3_gray, 0)
        self.fake_B3_gray = torch.unsqueeze(self.fake_B3_gray, 0)
        self.fake_B3 = torch.unsqueeze(self.fake_B3, 0)

        self.fake_B2 = torch.squeeze(self.fake_B2)
        r1, g1, b1 = self.fake_B2[0] + 1, self.fake_B2[1] + 1, self.fake_B2[2] + 1
        self.fake_B2_gray = 1. - (0.299 * r1 + 0.587 * g1 + 0.114 * b1) / 2.
        self.fake_B2_gray = torch.unsqueeze(self.fake_B2_gray, 0)
        self.fake_B2_gray = torch.unsqueeze(self.fake_B2_gray, 0)
        self.fake_B2 = torch.unsqueeze(self.fake_B2, 0)
        self.fake_B = torch.cat((self.fake_B3, self.fake_B2), dim=0)
        self.fake_B_gray = torch.cat((self.fake_B3_gray, self.fake_B2_gray), dim=0)
        self.fake_B1= self.netG_B.forward(self.fake_B, self.fake_B_gray)
        ...
        if self.opt.skip == 1:
            ...
            self.fake_B = self.netG_A.forward(self.real_img, self.real_A_gray)
            self.real_A_patch1, self.fake_B_patch1 = self.transforms_crop(self.real_A, self.fake_B)
            self.real_A_patch2, self.fake_B_patch2 = self.transforms_crop(self.real_A, self.fake_B)
            self.real_A_patch3, self.fake_B_patch3 = self.transforms_crop(self.real_A, self.fake_B)
            self.real_A_patch4, self.fake_B_patch4 = self.transforms_crop(self.real_A, self.fake_B)
            self.real_A_patch5, self.fake_B_patch5 = self.transforms_crop(self.real_A, self.fake_B)
            self.real_A_patch6, self.fake_B_patch6 = self.transforms_crop(self.real_A, self.fake_B)
            self.fake_B1_patch1, self.fake_B_patch1 = self.transforms_crop(self.fake_B1, self.fake_B)
            self.fake_B1_patch2, self.fake_B_patch2 = self.transforms_crop(self.fake_B1, self.fake_B)
            self.fake_B1_patch3, self.fake_B_patch3 = self.transforms_crop(self.fake_B1, self.fake_B)
            self.fake_B1_patch4, self.fake_B_patch4 = self.transforms_crop(self.fake_B1, self.fake_B)
            self.fake_B1_patch5, self.fake_B_patch5 = self.transforms_crop(self.fake_B1, self.fake_B)
            self.fake_B1_patch6, self.fake_B_patch6 = self.transforms_crop(self.fake_B1, self.fake_B)
    ...
    def backward_G(self, epoch):
        ...
        self.loss_idt = self.criterionIdt(self.fake_C, self.real_B) * 5
        if self.opt.use_wgan:
        ...
        if self.opt.vgg > 0:
            ...
            self.loss_G = 0.5 * self.loss_G_A + 0.5 * self.loss_vgg_b * vgg_w
        elif self.opt.fcn > 0:
        ...
        self.loss_phase_A1 = self.get_phase_loss(self.fake_B1, self.fake_B) / 7
        # phase patches loss
        self.loss_phase_A1_patch = (self.get_phase_loss(self.fake_B_patch1, self.fake_B1_patch1)
                                   + self.get_phase_loss(self.fake_B_patch2, self.fake_B1_patch2)
                                   + self.get_phase_loss(self.fake_B_patch3, self.fake_B1_patch3)
                                   + self.get_phase_loss(self.fake_B_patch4, self.fake_B1_patch4)
                                   + self.get_phase_loss(self.fake_B_patch5, self.fake_B1_patch5)
                                   + self.get_phase_loss(self.fake_B_patch6, self.fake_B1_patch6)) / 7
        self.loss_P1 = (self.loss_phase_A1+ self.loss_phase_A1_patch) * 0.0001
        # # quality loss
        self.loss_Q1 = self.get_quality_hyperIQA_loss(self.fake_B1, self.fake_A, epoch)*0.001
        # task loss
        self.loss_T1 = self.get_task_loss1(self.fake_B1, epoch) * 0.0001
        # self.loss_phase_A = self.get_phase_loss(self.real_A, self.fake_B) / 7
        #
        # self.loss_phase_A_patch = (self.get_phase_loss(self.fake_B_patch1, self.real_A_patch1)
        #                            + self.get_phase_loss(self.fake_B_patch2, self.real_A_patch2)
        #                            + self.get_phase_loss(self.fake_B_patch3, self.real_A_patch3)
        #                            + self.get_phase_loss(self.fake_B_patch4, self.real_A_patch4)
        #                            + self.get_phase_loss(self.fake_B_patch5, self.real_A_patch5)
        #                            + self.get_phase_loss(self.fake_B_patch6, self.real_A_patch6)) / 7
        # self.loss_P = (self.loss_phase_A + self.loss_phase_A_patch)*0.001
        #
        # self.loss_Q = self.get_quality_hyperIQA_loss(self.fake_B, self.real_B, epoch)*0.01

        # self.loss_T = self.get_task_loss(self.fake_B, epoch)*0.001
        self.loss_G = self.loss_G_A + self.loss_idt+self.loss_Q1+self.loss_P1+self.loss_T1
            ...
            if self.opt.patchD:
                ...
                if self.opt.patch_vgg:
                    ...
                    if not self.opt.self_attention:
                        return OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('fake_B1', fake_B1), ('latent_real_A', latent_real_A),
                                            ('latent_show', latent_show), ('real_B', real_B),('fake_A', fake_A),
                ...
                if not self.opt.self_attention:
                    return OrderedDict([('real_A', real_A), ('fake_B', fake_B),('fake_B1', fake_B1), ('latent_real_A', latent_real_A),('fake_A', fake_A),
                                        ('latent_show', latent_show), ('real_B', real_B)])
                else:
                    self_attention = util.atten2im(self.real_A_gray.data)
                    return OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('fake_B1', fake_B1),('real_B', real_B),('fake_A', fake_A),
