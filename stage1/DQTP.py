...
class SingleModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if not self.isTrain or opt.continue_train:
            ...
            self.load_network(self.netG_B, 'G_A', '200')
            self.load_network(self.netG_A, 'G_A', '220')


            if self.isTrain:

                self.load_network(self.netD_A, 'D_A', '220')
                if self.opt.patchD:
                    self.load_network(self.netD_P, 'D_P', '220')
        if self.isTrain:
            self.old_lr = opt.lr
    ...
    def get_quality_hyperIQA_loss(self, img1, img2, epoch):
        ...
        hyperIQA_score = abs(pred1 - pred2) /( pred2*3)
    ...
    def backward_G(self, epoch):
        ...
        self.loss_P = (self.loss_phase_A + self.loss_phase_A_patch)*0.0001

        self.loss_Q = self.get_quality_hyperIQA_loss(self.fake_B, self.real_B, epoch)*0.001

        self.loss_T = self.get_task_loss(self.fake_B, epoch)*0.0001
        self.loss_G = self.loss_G_A + self.loss_idt+self.loss_Q1+self.loss_P1+self.loss_T1+self.loss_P+self.loss_T+self.loss_Q
            ...
            if self.opt.patchD:
                ...
                if self.opt.patch_vgg:
                    ...
                    if not self.opt.self_attention:
                        return OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('latent_real_A', latent_real_A),
                                            ('latent_show', latent_show), ('real_B', real_B),('fake_B1', fake_B1),('fake_A', fake_A),
                ...
                if not self.opt.self_attention:
                    return OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('latent_real_A', latent_real_A),('fake_B1', fake_B1),('fake_A', fake_A),
                                        ('latent_show', latent_show), ('real_B', real_B)])
                else:
                    self_attention = util.atten2im(self.real_A_gray.data)
                    return OrderedDict([('real_A', real_A), ('fake_B', fake_B), ('real_B', real_B),('fake_B1', fake_B1),('fake_A', fake_A),
